...
(function fourchar(module, require) {
    ...
    var
        ...
        _prepareFile = function _prepareFile(fileDef, options, cb) {
            ...
            _fs.readFile(_utils.dir('/input/' + _fileDefName), { encoding: options.destEncoding }, function _onReadFile(err, data) {
                if(!!err) {
                    throw err;
                }

                var _fileDefNameDotIndex = _fileDefName.indexOf('.'),
                    _queryFilename = _utils.dir('/input/' + (_path.basename(_fileDefName, _fileDefNameDotIndex !== -1 ? _fileDefName.slice(_fileDefNameDotIndex) : '')));

                var _entries = _queryFile(data),
                    _lines = [
                        [
                            '　？？？ ',
                            _path.basename(_queryFilename).toUpperCase() + ' - ' + fileDef.desc,
                            //'Retrieved from <' + fileDef.origin.replace(/\//g, '%2F') + '>',
                            fileDef.attrib,
                            'Retrieved: ' + _utils.getDateNow()
                        ].join('/') + '/'
                    ];
                ...
                _fs.writeFile(_queryFilename, _lines.join('\n'), { encoding: options.destEncoding, flag: 'w' }, cb);
            });
        };
